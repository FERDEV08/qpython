...
sql = "SELECT ST_AsText (CastAutomagic(geom)) as geom,cve_ent, geografico,nomserv,tipo,ambito from servicios_informacion_complementaria_de_tipo_puntual where cve_ent = '01' and upper (geografico) = 'ESCUELA'"
...
#Se crea una lista en la cual se agregaran los campos que contendra la clase de tipo QgsVectorLayer (point layer)
listaQgsField = []
#Crea una clase del tipo QgsVectorLayer para almacenar datos vectoriales tipo punto en memoria
point_layer = QgsVectorLayer("Point?crs=epsg:6365", "Escuelas", "memory")
#Se instancea la clase dataprovider
pr = point_layer .dataProvider()
#listaQgsField.append (QgsField("id",Qvariant.Int))
listaQgsField.append (QgsField("cve ent",QVariant.String,'', 2, 0))
listaQgsField.append (QgsField("geografico”",QVariant.String, '', 59, 0))
listaQgsField.append (QgsField("nomserv",QVariant.String, '', 110, 0))
listaQgsField.append (QgsField("tipo",QVariant.String,'', 48, 8))
listaQgsField.append (QgsField("ambito",QVariant.String,'', 6, 0))
#Se agrega al dataProvider del nuestro clase point layer los atributos que tendra
pr.addAttributes(listaQgsField)

#Se actualiza la clase point layer con los atributos que se le agregaron
...
for row in rows: ##Itera sobre los resultados del la consulta
    ...
    geometria = QgsGeometry.fromWkt (row [0]) #se utiliza el metodo fromikt de la clase QgsGeometry para convertir el KT en geometria de QGIS
